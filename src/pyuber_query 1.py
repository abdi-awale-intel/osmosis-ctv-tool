...
def uber_request(indexed_input, test_name_file, test_type='',needed_suffix=False, output_folder='', program='DAC%', extra_identifier='', lot = ['Not Null'], wafer_id = ['Not Null'], prefetch = '1', databases = ['D1D_PROD_XEUS','F24_PROD_XEUS'],config_number = '',mode=''):
    ...
    if test_type == 'ClkUtils' or (needed_suffix and test_type != "SmartCtvDc"):
        ...
        token_names_list = token_names_fail + token_names_pass + token_names_missing + token_names_FAIL + token_names_PASS
    else:
        ...
        token_names_pass = [token_name + "_pass"  for token_name in token_names if not token_name.lower().endswith('_pass') and not token_name.lower().endswith('_fail')]
        token_names_fail = [token_name + "_fail"  for token_name in token_names if not token_name.lower().endswith('_pass') and not token_name.lower().endswith('_fail')]
        token_names_missing = [token_name for token_name in token_names if token_name.upper().endswith('_PASS') or token_name.upper().endswith('_FAIL')]
        token_names_list = token_names_fail + token_names_pass + token_names_missing + token_names_FAIL + token_names_PASS
        #token_names_string = "',\n'".join(token_names)
